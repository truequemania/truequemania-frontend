...
function Explorador() {
  ...
  const [articulos, setArticulos] = useState<Articulo[]>([]); // Estado con tipo correcto
  ...
  // Filtrar los artículos basándose en el searchTerm
  ...
  return (
    <div>
      ...
      <div className="bg-gradient-to-r from-gray-900 via-black to-gray-900 flex flex-col justify-between h-auto p-4 rounded-lg mt-14 shadow-md">
        {isOpenImg && <ArticulosFormImage toggleModalImagen={toggleModalImagen} />}
        ...
        <form className="w-full flex justify-center mt-6">
          <div className="relative w-3/4">
            <input
              ...
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)} // Filtrado en tiempo real
            />
          </div>
        </form>
